...
public class DownloadCSV {
    public static void downloadCSV(String url) throws IOException {
        ...
        // Copy the content from the URL to the local file and measure the time
        ...
        System.out.println("Download realizado com sucesso. Local: " + System.getProperty("user.dir") + "\\csvBaixado.csv");
        System.out.println("Tempo consumido para o download: " + elapsedTime + " milissegundos");

        // Count the number of records in the CSV
        int totalRecords = countCSVRecords(file);
        System.out.println("Quantidade total de registros no CSV: " + totalRecords);

        // Read the headers (assuming the first line contains column names)
        String[] headers = readCSVHeaders(file);

        // Find the index of the "List Year" column
        int listYearIndex = -1;
        for (int i = 0; i < headers.length; i++) {
            if ("List Year".equals(headers[i])) {
                listYearIndex = i;
                break;
            }
        }
        ...
        if (listYearIndex == -1) {
            System.out.println("Column 'List Year' not found.");
        } else {
            // Initialize a counter for values with "2020"
            int count2020 = count2020ValuesInColumn(file, listYearIndex);

            System.out.println("Number of '2020' values in the 'List Year' column: " + count2020);
        }
        
    }

    public static int countCSVRecords(File file) throws IOException {
        int totalRecords = 0;
    ...
    public static String[] readCSVHeaders(File file) throws IOException {
        BufferedReader reader = new BufferedReader(new FileReader(file));
    ...
    public static int count2020ValuesInColumn(File file, int columnIndex) throws IOException {
        int count2020 = 0;
        BufferedReader reader = new BufferedReader(new FileReader(file));
        ...
        while ((line = reader.readLine()) != null) {
            ...
            if (columnIndex < values.length && values[columnIndex].contains("2020")) {
                count2020++;
            }
        }
